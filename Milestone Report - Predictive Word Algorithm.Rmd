...
library(ggplot2); library(data.table); library(tidytext)
```
...
Additional steps that I could have taken include:
...
Since stop words are key words in the language excluding them wouldn't provide a very usefull prediction algorithm. While stemming the data is useful to create n-grams it provides to be a challenge in using stem words to predict a next word. Lastly, sparse words were kept to provide a more complete dictionary.
...
The model used to predict the next word make use of the N_grams created. Quite simply if the user enters a word of length n, the NextWordPrediction function looks for words in the (n+1)-gram that matches those inputs and returns the last word in the n-gram with the highest occurance. To enable learning, once a input is called we update proportions accordingly. If no matches are found, the function tries matching with n-first word until n = 1. If no matches are found we add this to the dataset. 

E.g:
Input = "she was"
n = 2
The NextWordPrediction function looks for words in the 3-gram (n+1) that matches those inputs and returns the last word in the 3-gram with the highest occurance and update proportions of input accordingly. If no matches are found, the function tries matching with input = "was". If no matches are found we add this to the dataset. 
...
As an attempt to evaluate the model I propose applying the NextWordPrediction function to predict the test set. Since the creation of N-grams in the test set creates all possible combinations of words that arises from the test set, we can apply the NextWordPrediction function to all but the last word in n-gram and compare the original n-gram to the resulting answer from NextWordPrediction. This evaluation yields an accuracy of 47%.

``` {r evaluation 1, cache = T}
test <- test %>%
        as.data.frame(.) %>%
        DataframeSource(.) %>%
        VCorpus(., readerControl = list(language = "en")) %>%
